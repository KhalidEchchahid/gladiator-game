...
// Fonction pour ajouter une image de fond � une fen�tre GTK
void add_bgimage(GtkWidget* window, GtkWidget** container, layout* bgimage) {
    GtkWidget* image;
    GdkPixbuf* pixbuf;
    GdkPixbuf* scaled_pixbuf;

    // Charge l'image depuis le chemin sp�cifi� dans la structure bgimage
    pixbuf = gdk_pixbuf_new_from_file(bgimage->image, NULL);

    // Redimensionne l'image si la largeur ou la hauteur sp�cifi�e est diff�rente de z�ro
    ...
    // Lib�re la m�moire utilis�e par le GdkPixbuf
    g_object_unref(pixbuf);

    // Ajoute l'image � un conteneur et la rend visible
    gtk_container_add(GTK_CONTAINER(*container), image);
    gtk_widget_show(image);

    // Met � jour la taille du conteneur pour s'adapter � l'image
    gtk_widget_set_size_request(*container, bgimage->width, bgimage->height);

    // Attache l'�v�nement "destroy" de la fen�tre � une fonction de rappel pour lib�rer la m�moire de l'image
...
Window* initializeWindow( gchar* name,gchar* title, gchar* icon, gint height, gint width, gint position, gint x, gint y, gint resizable, gchar* bgColor,gchar* bgimage,gint imagew,gint imageh) {
    ...
    window->title = NULL;
    window->icon = NULL;
    window->bgColor = NULL;  // Initialisez bgColor � NULL
...
void createWindow(Window* wo) {
    ...
    if (wo->bgColor)  // V�rifiez si bgColor est d�fini
        addBackgroundColooor(wo->window, wo->bgColor, 1.0);  // Utilisez bgColor pour d�finir la couleur de fond
